...
* **`config`**:
    * **`dl4miceveryhwere`**:
        ...
        * **`ubuntu_version`**: The Ubuntu version for the operating system in the Docker image. This version contributes to the logical Docker base-image tag (from [Nvidia](https://hub.docker.com/r/nvidia/cuda) if GPU is selected and from [Ubuntu](https://hub.docker.com/_/ubuntu) otherwise). DL4MicEverywhere resolves that tag through `.tools/base_images.lock.yaml` and builds from its immutable digest. The selected Ubuntu version must be among the supported tags.
